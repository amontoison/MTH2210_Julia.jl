...
``Y_{n+1} = Y_{n} + h F(t_n,Y_n)`` jusqu'au temps ``t_f`` avec pas constant
``h``
...
# Entrée
    1.  fct     -   Fonction le système de N EDOs
...
function euler(fct::Function , tspan::Vector{T},
    ...
    #@time for t=1:nbpas
    #    temp    =   Y[:,t]
    #    Y[:,t+1] .= view(Y,:,t) .+ lmul!(h,temp)
    #end

    #@time for t=1:nbpas
    #    @inbounds Y[:,t+1] .= view(Y,:,t) .+ h .* fct(temps[t], view(Y,:,t))
    #end
