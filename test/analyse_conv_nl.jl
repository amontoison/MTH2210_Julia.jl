...
println(lineplot(log.(10, err_bis1[err_bis1 .> 0]), name="Bissection", xlabel="k", ylabel="|eₖ₊₁|/|eₖ|"    , color=:red))
println(lineplot(log.(10, err_sec1[err_sec1 .> 0]), name="Sécante"   , xlabel="k", ylabel="(|eₖ₊₁|/|eₖ|)^α", color=:blue))
println(lineplot(log.(10, err_new1[err_new1 .> 0]), name="Newton"    , xlabel="k", ylabel="(|eₖ₊₁|/|eₖ|)²" , color=:yellow))
...
println(lineplot(log.(10, err_bis2[err_bis2 .> 0]), name="Bissection", xlabel="k", ylabel="|eₖ₊₁|/|eₖ|"    , color=:red))
println(lineplot(log.(10, err_sec2[err_sec2 .> 0]), name="Sécante"   , xlabel="k", ylabel="(|eₖ₊₁|/|eₖ|)^α", color=:blue))
println(lineplot(log.(10, err_new2[err_new2 .> 0]), name="Newton"    , xlabel="k", ylabel="(|eₖ₊₁|/|eₖ|)²" , color=:yellow))

# Vérification de Newton pour racines multiples
fct3(x)   = x * sin(x)^2
d_fct3(x) = sin(x)^2 + 2 * x * sin(x) * cos(x)

(approx_new3 , err_new3) = newton1D(fct3, d_fct3, 1.0, 200, 1e-12)
(approx_new4 , err_new4) = newton1D(fct3, d_fct3, 3.0, 200, 1e-12)
...
(approx_ptf5, err_ptf5) = ptfixes(fct4, 1.0, 200, 1e-12)
...
(approx_ptf6, err_ptf6) = ptfixes(fct5, 1.0, 200, 1e-12)
